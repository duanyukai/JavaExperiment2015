...
public class CatalogGUI extends JPanel {
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		dataFieldsPanel.setLayout(new GridLayout(dataFields.size(),2));

		for(DataField df : dataFields){
            JLabel label = new JLabel(df.getName() + ":");
			dataFieldsPanel.add(label);
            dataFieldsPanel.setBounds(30,30,100,100);

            JTextField textField = new JTextField(df.getValue());
            textField.setEnabled(false);
            textField.setDisabledTextColor(new Color(0,0,0));
            textField.setBackground(new Color(237,237,237));
			dataFieldsPanel.add(textField);
		}*/
        JPanel dataFieldsPanel = new JPanel();
        GridBagLayout layout = new GridBagLayout();
        dataFieldsPanel.setLayout(layout);
        ...
        textFieldConstraints.weightx = 10;
        ...
        for(DataField df : dataFields){
            ...
            layout.setConstraints(textField, textFieldConstraints);
            dataFieldsPanel.add(textField);
            j++;
        }
	...
	class CatalogListListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent event) {
            ...
            //DD edited
            productPanel.add(
                    "North",getDataFieldsPanel(product.getDataFields()));
